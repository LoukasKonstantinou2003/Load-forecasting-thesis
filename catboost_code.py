...
METRICS_FILE = METRICS_DIR / "catboost_70_30_metrics.csv"

SVG_DIR = Path("catboost_svg")
...
def plot_load_timeseries(
    ...
    plt.xlabel("Date")
    plt.ylabel("Load")
    plt.title(f"CatBoost: {dataset_name} – Load over Time")
    plt.tight_layout()
...
def train_size_experiment(
    ...
    plt.ylabel("nRMSE")
    plt.title(f"CatBoost: {dataset_name} – nRMSE vs Training Size (Test={test_pct}%)")
    plt.tight_layout()
...
def train_model_70_30(
    ...
    plt.xlabel("Date")
    plt.ylabel("Load")
    plt.title(f"CatBoost: {dataset_name} – Actual vs Predicted (70:30)")
    plt.legend()
    plt.tight_layout()
